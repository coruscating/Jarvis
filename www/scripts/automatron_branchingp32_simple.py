...
for iii in range(1,500):
            ...
            while self.running==1:
                ...
                if os.path.isfile(filedir + "branchingratio_" + fileprefix + "_" + str(iii)) is True: # don't overwrite
                    ...
                    checksum=sum([i[1] for i in readout[1:1000]])
                    self.server.speak("Done, %d counts from 408" %(checksum))
                    if checksum<6000:
                        self.server.speak("No 408!")
                        sys.exit()
